...
// 上层变了，依赖它的下层必须跟着变——不需要显式列举「改了 A 要连带
	tests := []struct {
		...
		},
		{
			name: "改设备自己的 rootfs",
			mutate: func(t *testing.T, root string) {
				write(t, root, "devices/router/rootfs/etc/sysctl.d/99.conf", "y\n")
			},
			changedVariant: []string{"router@25.12", "router@25.12-mtk"},
		},
		{
			name: "改所有设备共用的 rootfs 层",
			mutate: func(t *testing.T, root string) {
				write(t, root, "rootfs/etc/banner", "换了\n")
			},
			changedVariant: []string{"router@25.12", "router@25.12-mtk", "vm@25.12"},
		},
		{
...
// 这一条是上一代真实存在的洞：那时 firmware 指纹哈希整棵
	...
	for _, tc := range tests {
		t.Run(tc.name, func(t *testing.T) {
			...
			assertChanged(t, "packages", before, after, tc.changedPackages, func(f Fingerprints) string { return f.Feed })
