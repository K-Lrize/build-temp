...
func gcCmd(app *App) *cobra.Command {
	var keep int
	var threshold int
	var apply bool
	var force bool
	...
	cmd := &cobra.Command{
		Use:   "gc",
		Short: "引用计数回收 R2 旧产物（默认 dry-run，超阈值熔断）",
		Args:  cobra.NoArgs,
		RunE: func(cmd *cobra.Command, args []string) error {
			var pins gcPins
			for _, p := range pinList {
				if err := pins.Set(p); err != nil {
					return err
				}
			}
			...
			if err := gcReleases(ctx, app, cl, variants, keep, pins, st); err != nil {
				return err
			}
	...
	cmd.Flags().IntVar(&keep, "keep", 3, "每 (设备,线) 保留最新 N 个 release")
	cmd.Flags().IntVar(&threshold, "threshold", 30, "单次删除比例超过这个百分比就熔断")
	cmd.Flags().BoolVar(&apply, "apply", false, "真的删除（默认 dry-run，只打印计划）")
	cmd.Flags().BoolVar(&force, "force-over-threshold", false, "熔断后仍强制执行")
	cmd.Flags().StringSliceVar(&pinList, "pin", nil, "钉住不删的 release，形如 <device>@<line>:<release_id>")
	return cmd
}
...
func gcReleases(ctx context.Context, app *App, cl *publish.Client, variants []resolve.Variant, keep int, pins gcPins, st *gcState) error {
	fmt.Fprintf(app.Stdout, "=== 固件 release（每设备每线保留最新 %d + 被 pin 的） ===\n", keep)
	for _, d := range distinctDeviceLines(variants) {
		...
		fmt.Fprintf(app.Stdout, "  %s@%s: 现存 %d，清理 %d\n", d.device, d.line, len(rids), len(del))
